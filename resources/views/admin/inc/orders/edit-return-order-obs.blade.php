...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        تعديل
        <small>تعديل سبب إرجاع الطلب</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{route('admin.dashboard')}}"><i class="fa fa-dashboard"></i> الرئيسية</a></li>
        <li class="active">تعديل سبب إرجاع الطلب</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="box box-info">
        <div class="box-header">
          <h3 class="box-title">تعديل <small>سبب إرجاع الطلب</small></h3>
          <!-- tools box -->
        ...
        <div class="box-body pad">
          <form action="{{route('admin.return.order.observation.update')}}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="obs_id" value="{{$obs->id}}">
            <div class="form-group {{$errors->has('obs')?'has-error':''}}">
                <label for="obs"></label>
                <input type="text" name="obs" class="form-control" placeholder="أكتب سبب إلغاء الطلب" value="@if(old('obs')){{old('obs')}}@else{{$obs->obs}}@endif">
                @if($errors->has('obs'))
